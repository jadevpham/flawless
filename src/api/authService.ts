...
const authService = {
  ...
  googleSignIn: async (googleToken: string) => {
    try {
      ...
      const user = {
        id: decodedUser.sub, // hoặc data.id nếu BE trả về, sub thường là user id
        fullname: decodedUser.name || data.name || "No Name",
        email: decodedUser.email || data.email || "",
        role:
          decodedUser[
            "http://schemas.microsoft.com/ws/2008/06/identity/claims/role"
          ] || data.role || "Customer",
        phone: "", // nếu BE không có thì để rỗng
        gender: "unknown", // nếu BE không có thì default
        dob: "", // nếu BE không có thì default
      };
      ...
      return {
        ...
        role: user.role,
        isSuccess: true,
        name: user.fullname, // để bạn show toast
        email: user.email,
      };
    } catch (error: any) {
