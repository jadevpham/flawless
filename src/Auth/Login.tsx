...
import { Eye, EyeOff, User as UserIcon, Lock, Sparkles } from "lucide-react";
...
import type {loadUserProfile} from "../utils/loadUserProfile";
...
const Login: React.FC = () => {
  ...
  try {
    ...
    if (res?.isSuccess && res.token) {
		const user: User = {
			id: '', // API không trả id → gán tạm ''
			fullname: res.name ?? '',
			email: res.email ?? '',
			role: res.role ?? '',
		  };
		
      setAuth(res.token, res.refreshToken, user, res.role ?? '');
      toast.success(`Xác thực thành công. Chào mừng, ${res.name}!`);

	  // Đợi isSyncingCurrentUser === false
	  const waitForSync = async () => {
		while (store.getState().auth.isSyncingCurrentUser) {
			console.log("Waiting for sync...");
			await new Promise((resolve) => setTimeout(resolve, 100));
		}
		console.log("Sync done, currentUser:", store.getState().auth.currentUser);
	};
	
console.log("isSyncingCurrentUser:", typeof store.getState().auth.isSyncingCurrentUser, store.getState().auth.isSyncingCurrentUser);
														...
														InputProps={{
															startAdornment: (
																<InputAdornment position="start">
																	<UserIcon  className="text-[#BD9EB7] w-5 h-5" />
																</InputAdornment>
															),
															style: { height: "56px" },
														}}
