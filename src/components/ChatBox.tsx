...
import { handleHybridChat } from "@/services/chatService";
...
export default function Chatbox() {
      ...
      if (parsed.length === 0) {
        dispatch(
          addUserMessage({
            role: "assistant",
            content:
              "Xin chào! Bạn có thắc mắc gì về Flawless không? Mình có thể hỗ trợ bạn ngay.",
          })
        );
      } else {
        dispatch(setMessagesFromHistory(parsed.slice(-50)));
      }
    } else {
      dispatch(
        addUserMessage({
  ...
  const handleSendMessage = () => {
    ...
    if (now - lastSentTime.current < 2000) {
      alert("Vui lòng chờ một chút trước khi gửi tiếp.");
      return;
    }

    dispatch(setLoading(true));
    dispatch(handleHybridChat(input, role)); // ✅ Gọi API chat (sẽ tự thêm tin nhắn người dùng)
    setInput("");
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter") {
      ...
      handleSendMessage(); // ✅ Thêm gọi hàm gửi tin nhắn khi nhấn Enter
    }
  };
  ...
  return (
    <>
      <button
        className="fixed bottom-5 left-5 hover:text-[#6A3471] text-white p-3 rounded-full shadow-lg bg-[#9C62A0]"
      ...
      <div
        className={`fixed bottom-16 left-5 w-[450px] h-[550px] bg-white shadow-2xl rounded-2xl border border-gray-300 transition-all z-50 ${
      ...
      >
        <div className="p-4 border-b bg-[#B77DBA] text-white rounded-2xl flex justify-between items-center">
          ...
          <button onClick={() => setIsOpen(false)}>
            <X size={24} className="text-[#B77DBA]" />
          </button>
        </div>
  ...
  <div
    ...
    className={`p-3 rounded-lg max-w-[80%] ${
      msg.role === "user"
        ? "bg-[#FCF5FA] text-black self-end ml-auto"
  ...
  >
    {msg.type === "image" ? (
      ...
      <p className="whitespace-pre-line">{msg.content}</p>
        ...
        <div className="flex p-3 border-t bg-gray-100 rounded-b-lg">
          <input
